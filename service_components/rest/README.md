...
2) Запустіть Node-RED. Якщо Ви не працювали до цього з Node-RED можете пройти для початку [цю лабораторну роботу](https://pupenasan.github.io/ProgIngContrSystems/%D0%9B%D0%B0%D0%B1%D0%BE%D1%80/lab1_nodered.html)

### 2. Створення фрагменту автентифіації

1. Прочитайте про автентифікацію в інтерфейсі REST data, що наведена вище.
...
- Потік у фрагменті запускається автоматично, за рахунок відповідного налаштування вузлу `inject`. У корисне навантаження вставляється та частина url (префікс), яка буде незмінна для всіх запитів. 
- Далі у наступному вузлі відбувається (див. Запит маркера автентифікації) :
  - збереження частини url в контексті потоку, щоб потім його використовувати в інших частинах потоку
  - формування msg.url для подачі його в http-запит: до префіксу добавляється `/auth/auth-token`
...
- після обробки першого вузла `http request` в потоці при позитивному результатів на виході сформується відповідь з потрібним `code` (див. Запит маркера автентифікації)
...
**Зверніть увагу, що маркер надається на певний час. Тому вузол ініціювання отримання маркеру треба буде інколи запускати вручну. Можна змінити програму, щоб він перезапускався кожні 5 хв, тоді маркер буде постійно свіжий.**  

### 3. Отримання словника даних

1. Прочитайте про отримання словника даних в інтерфейсі REST data, що наведена вище.

2. Добавте на існуючому потоці фрагмент програми наведений нижче на рисунку. Він повертає JSON з усіма доступними змінними та інформацією про їх тип. Під рисунком опис налаштувань усіх вузлів. Експортний варіант фрагменту наведений в коді нижче:
...
2. Добавте на існуючому потоці фрагмент програми наведений нижче на рисунку. Він реалізує читання змінної. Під рисунком опис налаштувань усіх вузлів. Експортний варіант фрагменту наведений в коді нижче:  
...
- Також у вузлі Read Tags за допомогою виразів JSONata формується url з префіксу та запитуваної змінної
...
3) Зробіть розгортання проекту. Натисніть кнопку на вузлі Inject, результат має бути як на рисунку нижче. Зверніть увагу на значення змінної, порівняйте його з тим, що на ВЕБ-сторінці ПЛК. 
...
1. Прочитайте про запити на запис змінних в інтерфейсі REST data, що наведена вище.
