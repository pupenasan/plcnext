...
В кінці розділу є приклад використання REST PLCnext з клієнта для читання та запису змінних в ПЛК, реалізованого в:

- Node-RED
- тестових утиліт на базі браузера 
...
## Приклад тестування клієнта через утиліти 

Тут для демонстрації роботи REST API в PLCnext використаємо в якості клієнта тестові утиліти для тестування. 
...
1) Для початку необхідно підготовити проект з однією змінною, який описано в швидкому старті  за [цим посиланням](https://pupenasan.github.io/plcnext/hmi/hmiquickstart.html). Завантажте його в імітатор ПЛК та перевірте що він працює.
2) Необхідно встановити доповнення для браузера, у прикладі використовується розширення `RESTED` яке є як для FireFox так і для Chrome. 

### 2. Автентифікація

Враховуючи що підключення відбувається за HTTPS за самопідписаним сертифікатом, спочатку треба відкрити в одній із вкладок браузера Web HMI та погодитися на ризик використання самопідписаного сертифікату.

- [ ] Зайдіть на сторінку Веб-серверу PLCnext `https://127.0.0.1:5050`
- [ ] Погодьтеся на ризик використання самопідписаного сертифікату
- [ ] Відкрийте розширення `RESTED`

**Для отримання маркеру автентифікації:**

Спочатку необхідно отримати маркер автентифікації:

- [ ] Виберіть метод POST та вкажіть:

- url: `https://127.0.0.1:5050/_pxc_api/auth/auth-token`
- type: `JSON`
- body: 
  - scope : "variables"

- [ ] Натисніть `Send Request` . Повинна прийти відповідь з кодом 

![image-20221031153028545](media/image-20221031153028545.png)

- [ ] Натисніть `+` для добавлення запиту в колекцію запитів для подальшого використання 

**Запит на автентифікацію користувача**

Тепер отриманий маркер автентифікації вставляється в тіло повідомлення запиту автентифікації як `code`. При цьому передається назва користувача та пароль.

- [ ] Заповніть поля в новому запиті:

- метод: `POST`

- url: `https://127.0.0.1:5050/_pxc_api/auth/access-token`
- type: `JSON`
- body: 

```json
{"code": "отриманий вище код", 
 "grant_type": "authorization_code", 
 "username": "admin", 
 "password": "plcnext"}
```

![image-20221031154123201](media/image-20221031154123201.png)

- [ ] Натисніть `Send Request` . Ви повинні отримати позитивну відповідь у наступному форматі:

![image-20221031154305273](media/image-20221031154305273.png)

Значення `acces_token` необхідно буде для формування іншого повідомлення:

- [ ] Натисніть `+` для добавлення запиту в колекцію запитів для подальшого використання 

### 3. Читання змінної

Тепер необхідно прочитати змінну `RealVar`. Зверніть увагу що в заголовок `Authorization` необхідно вказати отриманий перед цим `access_token`. 

- [ ] Заповніть поля в новому запиті:

- метод: `GET`

- url: `https://127.0.0.1:5050/_pxc_api/api/variables?pathPrefix=Arp.Plc.Eclr/&paths=RealVar`
- type: `JSON`
- headers: 

```json
{"Authorization":"Bearer ориманий_access_token"	}
```

![image-20221031160820450](media/image-20221031160820450.png)

- [ ] Натисніть `Send Request` . При позитивній відповіді ви отримаєте значення змінної у наступному форматі:

![image-20221031160921759](media/image-20221031160921759.png)

Якщо відповідь буде у форматі:

![image-20221031161946244](media/image-20221031161946244.png)

то це може свідчити про завершення терміну використання маркеру, тому необхідно знову:

- пройти 2-крокову автентифікацію
- вставити отриманий маркер в запит на читання

### 4. Запис змінної

Тепер необхідно записати змінну `RealVar`. Зверніть увагу що в заголовок `Authorization` необхідно вказати отриманий перед цим `access_token`. 

- [ ] Заповніть поля в новому запиті:

- метод: `PUT`

- url: `https://127.0.0.1:5050/_pxc_api/api/variables`
- type: `JSON`
- headers: 
...
![image-20221031163016173](media/image-20221031163016173.png)

- [ ] Натисніть `Send Request`. При позитивній відповіді Ви отримаєте значення змінної у наступному форматі:

![image-20221031163139687](media/image-20221031163139687.png)

Якщо відповідь буде у форматі "AccessDenied", то очевидно Вам повторно треба пройти автентифікацію.
